...
- [ ] Task 1: 初始化 Vite + React + TypeScript 项目，配置 Tailwind CSS
...
- [ ] Task 5: 配置 Supabase 项目与数据库
  - 在 Supabase 控制台创建项目
  - 创建数据库表：todos、pomodoro_records、notes、user_settings（含 RLS 策略）
  - 启用 Email Auth
  - 记录项目 URL 和 anon key

- [ ] Task 6: 实现认证页面与 Auth 逻辑
...
- [ ] Task 7: 定义 TypeScript 类型与 Supabase 操作封装
  - 创建 `types/todo.ts`、`types/pomodoro.ts`、`types/note.ts`
  - 创建 `lib/supabase.ts` 封装 CRUD 操作（按 `user_id` 过滤）
  - 创建 Zustand stores：`useTodoStore`、`usePomodoroStore`、`useNotesStore`
  - Store 从 Supabase 加载数据，修改时同步写入
  - 验证：Store 数据正确读写

## 阶段四：待办事项页

- [ ] Task 8: 实现待办分类系统与输入组件
...
- [ ] Task 16: 实现统计页面
  ...
  - `SummaryCards`：4 个概览指标卡片
  - `TrendChart`：Recharts 柱状图，X 轴按粒度变化
  - `TaskDistribution`：任务列表 + 饼图（Recharts）
  - `TaskCompletionCard`：完成情况表格
  - 空数据状态显示
  - 验证：粒度切换正常，图表数据正确
